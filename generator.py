...
def do_the_thing(input_file_path, output_file_path, container_header_name, output_mode):
    lines = []
    ...
    if (output_mode == "HEADERS"):        
        ...
        for i in range(1, len(headers)):
            if headers[i].startswith("STRING"):
                cpp_header += f"extern ankerl::unordered_dense::map<dcon::{object_name}_id::value_base_t, std::string> {headers[i][7:]};\n"
...
do_the_thing(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4])
